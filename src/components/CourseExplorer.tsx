...
import { PlayCircle, Search, Sparkles, BookOpen } from 'lucide-react';
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-violet-400 mb-1">
            ...
            <span>Structured Video Masterclasses</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white">
            Video Courses & Academy
          </h1>
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            High-definition video courses with practical lessons, preview streams, and instant unlock.
          </p>
        </div>
        ...
        <div className="w-full md:w-80 relative">
          ...
          <input
            ...
            className="w-full bg-slate-900 border border-slate-800 focus:border-violet-500 rounded-xl pl-9 pr-4 py-2 text-xs text-slate-200 placeholder-slate-500 outline-none"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
          ...
          <button
            ...
            className={`shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
              selectedCategory === 'all'
                ? 'bg-violet-600 text-white font-bold shadow-sm shadow-violet-600/30'
          ...
          <div className="inline-flex rounded-lg bg-slate-950 p-0.5 border border-slate-800">
            <button
              ...
              className={`px-2.5 py-1 text-xs font-semibold rounded-md transition-all ${
                accessType === 'all' ? 'bg-violet-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              All
            </button>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="h-80 rounded-2xl bg-slate-900/40 border border-slate-800 animate-pulse p-4" />
          ))}
        </div>
      ) : courses.length > 0 ? (
        <div className="space-y-3">
          <p className="text-xs text-slate-400">Showing {courses.length} video course{courses.length === 1 ? '' : 's'}</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {courses.map((course) => (
        ...
        <div className="text-center py-16 px-4 rounded-3xl bg-slate-900/30 border border-slate-800">
          ...
          <p className="text-xs text-slate-400 max-w-sm mx-auto mt-1 mb-4">
            No courses found matching your criteria. Try adjusting the category or search query.
          </p>
          <button
            onClick={() => {
              setSearch('');
            ...
            className="px-4 py-2 rounded-xl bg-violet-600 text-white text-xs font-bold hover:bg-violet-500 transition-all"
          >
            Reset Filters
          </button>
        </div>
